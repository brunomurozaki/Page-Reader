...
Os padrões definidos pela \citeonline{w3cWai} são muito subjetivos. Muito porque na verdade, cada tipo de inabilidade necessita de uma atenção diferente. Este fator, somado com a extrema dificuldade de se padronizar páginas WEB para interpretação e utilização do conteúdo principal mostra que a busca por uma universalidade no acesso a informação é inviável. Há a possibilidade, porém não é vantajoso realizar trabalhos para alcançar tal feito. Portanto, estudos individuais para cada inabilidade devem ser feitos, para posteriormente, encontrar talvez, um ponto de intersecção para estes usuários.
